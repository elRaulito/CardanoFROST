...
export default function Page() {
  ...
  /* --- state (your original state kept intact) --- */
  ...
  useEffect(() => {
    (async () => {
      try {
        const mod = await import('../public/pkg/frost_pallas_wasm');
  ...
  /* ---------- your original handlers (unchanged except styling) ---------- */

  const generate = () => {
    try {
      const out = wasm.frost_generate_state(max, min);
      setStateJson(out);
  ...
  const parseTx = async () => {
    try {
      ...
      throw new Error('Provide Eternl JSON (paste) or upload a JSON/CBOR file.');
    } catch (e: any) {
  ...
  const makeCommitments = () => {
    try {
      ...
      const out = wasm.round1_make_commitments(shareJsonOne);
      const obj = typeof out === 'string' ? JSON.parse(out) : out;
      ...
      //downloadText(`nonces_b64_${String(obj.identifier)}.txt`, obj.nonces_b64);
  ...
  const onSigningPackageFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    try {
      ...
      setError(`Signing package upload failed: ${err?.message || String(err)}`);
    }
  };
  ...
  const signMyShare = () => {
    try {
      if (!shareJsonOne) throw new Error('Upload your SecretShare JSON first.');
      if (!noncesB64) throw new Error('Generate nonces/commitments first.');
      if (!signingPackageB64.trim()) throw new Error('Paste signing_package_b64 from coordinator.');
      const out = wasm.round2_sign_share(shareJsonOne, noncesB64, signingPackageB64.trim());
      const obj = typeof out === 'string' ? JSON.parse(out) : out;
      ...
      setError(`Sign share failed: ${e?.message || String(e)}`);
    }
  };
  ...
  function normalizeCommitmentEntry(text: string) {
    try {
      ...
      if (obj && obj.identifier && obj.commitments_b64) {
        return { identifier: String(obj.identifier), commitments_b64: String(obj.commitments_b64) };
      }
    } catch {}
    throw new Error('Commitment file must be a JSON object with {identifier, commitments_b64}.');
  }
  ...
  const onCommitmentsFiles = async (e: React.ChangeEvent<HTMLInputElement>) => {
    try {
      ...
      setError(`Commitments upload failed: ${err?.message || String(err)}`);
    }
  };
  ...
  const buildSigningPackage = () => {
    try {
      ...
      setError(`Build signing package failed: ${e?.message || e}`);
    }
  };
  ...
  const aggregate = async () => {
    try {
      ...
      if (!signingPackageB64Agg?.trim()) throw new Error('Paste signing_package_b64.');
      if (!sigShareFiles.length) throw new Error('Upload signature_share JSON files.');
      if (!eternlJson.trim() && !txHex.trim()) throw new Error('Provide Eternl JSON or a CBOR-parsed tx.');
      ...
      const out = wasm.aggregate_and_witness_from_eternl_with_pubkey(
        pubkeyPkgJson.trim(),
      ...
      if (obj.witness_set_hex) downloadText('witness_set.hex.txt', obj.witness_set_hex);
      if (obj.signed_tx_hex) downloadText('signed_tx.hex.txt', obj.signed_tx_hex);
      if (obj.signature_hex) downloadText('group_signature.hex.txt', obj.signature_hex);
    } catch (e: any) {
  ...
  return (
    <div className="mx-auto max-w-6xl px-5 py-10">
      <header className="mb-6">
        ...
        >
          FROST Cardano multisig <span className="text-amber-600">t-of-n (BETA, currently supporting only trusted setup with dealer)</span>
        </motion.h1>
        <p className="text-sm text-zinc-600">
          Threshold Ed25519 signing for Cardano. Use the tabs to act as{' '}
          <span className="font-medium text-amber-700">Dealer</span>,{' '}
          <span className="font-medium text-sky-700">Signer</span>, or{' '}
          <span className="font-medium text-emerald-700">Coordinator</span>.
        </p>
      </header>

      {/* Tabs */}
      <div className="mb-6 inline-flex rounded-2xl bg-white/70 p-1 shadow-sm ring-1 ring-zinc-200 backdrop-blur">
        {tabs.map(t => {
          const is = activeTab === t;
          const color =
            t === 'Dealer' ? 'amber' : t === 'Signer' ? 'sky' : 'emerald';
          return (
            <button
              key={t}
              onClick={() => setActiveTab(t)}
              className={[
                'relative mx-1 rounded-xl px-4 py-2 text-sm font-medium transition',
                is
                  ? `bg-gradient-to-br from-${color}-500 to-${color}-600 text-zinc-900 shadow ring-1 ring-${color}-300`
